...
static Header *morecore(size_t nu) {	// ask system for more memory
	char *cp, *sbrk(int);
	Header *up;
...
void *mymalloc(size_t size)
{
	...
	if ((prevp = freep) == NULL) {			// no free list yet, a degenerate free list will be created
		base.s.size = 0;					// degenerate free list contain one block of size zero
		base.s.ptr = freep = prevp = &base;	// and pointer to itself
	}
	for (p = prevp->s.ptr; ; prevp = p, p = p->s.ptr) {	// find for the next allocated block that have a size bigger than nunits
...
void *mycalloc(size_t nmemb, size_t size)
{
	...
	if (ptr != NULL) {	// clear the memory on successful alloction
